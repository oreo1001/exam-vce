...
import { getStudyStats, getWrongQuestions } from '@/lib/storage'

export default function HomePage() {
  const [stats, setStats] = useState({ answered: 0, correct: 0, total: 156 })
    ...
    <div className="min-h-full bg-gray-50 dark:bg-gray-950 py-12 px-4">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-lg">문제집 · 156문제</p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6 mb-8 shadow-sm">
          ...
          <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
            <div
              className="bg-[#ff3621] h-2 rounded-full transition-all duration-500"
              style={{ width: `${(stats.answered / stats.total) * 100}%` }}
            />
          </div>
          <p className="text-xs text-gray-400 mt-2 text-right">
            {stats.total - stats.answered}문제 남음
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4 mb-6">
            ...
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 shadow-sm hover:shadow-md hover:border-[#ff3621] dark:hover:border-[#ff3621] transition-all duration-200 group-hover:scale-[1.02]">
              ...
              <ul className="space-y-1 text-sm text-gray-600 dark:text-gray-400">
                <li className="flex items-center gap-2">
                  <span className="text-green-500">✓</span> 즉각적인 정답 피드백
                </li>
                <li className="flex items-center gap-2">
                  <span className="text-green-500">✓</span> 해설 확인 가능
                </li>
                <li className="flex items-center gap-2">
                  <span className="text-green-500">✓</span> 진행 현황 저장
                </li>
              </ul>
            </div>
          </Link>
          ...
          <Link href="/test" className="group block">
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 shadow-sm hover:shadow-md hover:border-[#ff3621] dark:hover:border-[#ff3621] transition-all duration-200 group-hover:scale-[1.02]">
              ...
              <ul className="space-y-1 text-sm text-gray-600 dark:text-gray-400">
                <li className="flex items-center gap-2">
                  <span className="text-orange-500">✓</span> 타이머 카운트다운
                </li>
                <li className="flex items-center gap-2">
                  <span className="text-orange-500">✓</span> VCE 스타일 성적표
                </li>
                <li className="flex items-center gap-2">
                  <span className="text-orange-500">✓</span> 문제 범위 선택 가능
                </li>
              </ul>
            </div>
          </Link>
        ...
        {/* Wrong answers link */}
        {wrongCount > 0 && (
          <Link href="/wrong" className="block">

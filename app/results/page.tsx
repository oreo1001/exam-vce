...
import { getTestResults, setWrongQuestions } from '@/lib/storage'
import { questions } from '@/lib/questions'

export default function ResultsPage() {
  const router = useRouter()
  const [results, setResults] = useState<ReturnType<typeof getTestResults>>(null)
    ...
    return (
      <div className="min-h-full bg-gray-50 dark:bg-gray-950 flex items-center justify-center py-12 px-4">
        <div className="text-center">
          ...
          <Link
            href="/test"
            className="px-5 py-2 rounded-xl bg-[#ff3621] text-white font-semibold hover:bg-[#cc2b1a] transition-colors"
          >
            시험 보러 가기
          </Link>
        </div>
  ...
  const scoreRaw = total > 0 ? correct / total : 0
  const score1000 = Math.round(scoreRaw * 1000)
  ...
  const elapsedStr = [
    elapsedH > 0 ? `${elapsedH}시간` : null,
    elapsedM > 0 ? `${elapsedM}분` : null,
    `${elapsedS}초`,
  ]
    .filter(Boolean)
    .join(' ')
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden">
          ...
          <div className="bg-gray-900 dark:bg-gray-950 px-6 py-6 text-white">
            ...
            <p className="text-gray-400 text-sm">Databricks Certified Data Engineer Associate</p>
          </div>
          ...
          >
            ...
            <div>
              ...
              <p
                className={`text-sm ${
                  passed ? 'text-green-600 dark:text-green-500' : 'text-red-500 dark:text-red-500'
                }`}
              >
                {passed ? '합격! 축하합니다!' : `불합격. 합격선 ${passingScore}점 미달`}
              </p>
            </div>
          </div>
          ...
          <div className="px-6 py-6 border-t border-gray-100 dark:border-gray-800">
            ...
            <div className="mb-4">
              <div className="flex justify-between text-xs text-gray-500 dark:text-gray-400 mb-1">
                <span>내 점수</span>
                <span className={`font-bold text-sm ${passed ? 'text-green-600 dark:text-green-400' : 'text-red-500 dark:text-red-400'}`}>
                  {score1000} / 1000
                </span>
              </div>
              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-6 overflow-hidden relative">
                ...
                >
                  {yourPct > 15 && (
                    <span className="text-white text-xs font-bold">{score1000}</span>
                  )}
                </div>
                {/* Passing line */}
                <div
                  ...
                  style={{ left: `${passPct}%` }}
                >
                  <div className="absolute -top-5 left-1 text-xs text-yellow-500 dark:text-yellow-400 whitespace-nowrap font-medium">
                    합격 {passingScore}
                  </div>
                </div>
              </div>
            ...
            <div>
              <div className="flex justify-between text-xs text-gray-500 dark:text-gray-400 mb-1">
                <span>합격 기준</span>
                <span className="font-medium">800 / 1000 (80%)</span>
              </div>
          ...
          <div className="px-6 py-4 border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-900">
            ...
            <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 mt-2">
              <div
                className={`h-full rounded-full ${passed ? 'bg-green-500' : 'bg-[#ff3621]'}`}
          ...
          <div className="px-6 py-5 border-t border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row gap-3">
            ...
            <Link
              href="/study"
              className="flex-1 py-2.5 rounded-xl font-semibold text-sm text-center bg-[#ff3621] text-white hover:bg-[#cc2b1a] transition-colors"
            >
              학습 모드로
            </Link>
          </div>

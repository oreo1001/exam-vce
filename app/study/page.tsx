...
import { getStudyAnswers, setStudyAnswer, StudyAnswers, addWrongQuestion, removeWrongQuestion, getWrongQuestions } from '@/lib/storage'
...
export default function StudyPage() {
  ...
  // Restore state when navigating to a question that already has an answer
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowLeft') handlePrev()
        ...
        if (['A', 'B', 'C', 'D', 'E'].includes(letter)) {
          const hasChoice = question.choices.some((c) => c.letter === letter)
          if (hasChoice) handleSelect(letter)
        }
      }
    }
          ...
          <div className="max-w-3xl mx-auto">
            ...
            <div className="flex gap-4 mt-3 text-xs text-gray-500 dark:text-gray-400">
              <span className="flex items-center gap-1">
                <span className="w-3 h-3 rounded-sm bg-green-500 inline-block" /> 정답
              </span>
              <span className="flex items-center gap-1">
                <span className="w-3 h-3 rounded-sm bg-red-500 inline-block" /> 오답
              </span>
              <span className="flex items-center gap-1">
                <span className="w-3 h-3 rounded-sm bg-gray-300 dark:bg-gray-700 inline-block" /> 미풀이
              </span>
            </div>
          </div>
        </div>

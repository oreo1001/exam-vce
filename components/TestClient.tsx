...
import { SubjectStorage, StudyAnswer } from '@/lib/storage'
...
export default function TestClient({ questions, storage, homeHref, resultsHref, examTitle }: TestClientProps) {
  ...
  const handleSubmit = useCallback(() => {
    ...
    storage.setTestResults({ answers, elapsed, total: quizQuestions.length, startTime })
    router.push(resultsHref)
  }, [submitted, answers, startTime, quizQuestions.length, storage, router, resultsHref])

  const handleTick = useCallback((remaining: number) => { setRemainingSeconds(remaining) }, [])
  const handleExpire = useCallback(() => { handleSubmit() }, [handleSubmit])

  function startQuiz() {
    const start = Math.max(1, rangeStart)
    const end = Math.min(questions[questions.length - 1].num, rangeEnd)
  ...
  const q = quizQuestions[currentIndex]
  const maxNum = questions[questions.length - 1]?.num ?? questions.length
        ...
        <div className="max-w-lg w-full">
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm p-8">
            <div className="flex items-center gap-3 mb-2">
              <a href={homeHref} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 text-sm transition-colors">← 홈</a>
            </div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">시험 설정</h1>
            <p className="text-gray-500 dark:text-gray-400 mb-8 text-sm">시험 범위와 제한 시간을 설정하세요</p>

            <div className="space-y-6">
              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">문제 범위</label>
                <div className="flex items-center gap-3">
                  <div className="flex-1">
                    <label className="text-xs text-gray-500 dark:text-gray-400 mb-1 block">시작</label>
                    <input type="number" value={rangeStart} min={1} max={maxNum}
                  ...
                  <div className="flex-1">
                    ...
                    <input type="number" value={rangeEnd} min={1} max={maxNum}
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">제한 시간</label>
                <div className="flex items-center gap-3">
                  <input type="number" value={timerMinutes} min={5} max={300}
                ...
                <div className="flex gap-2 mt-2">
                  {[60, 90, 120, 180].map((min) => (
                    <button key={min} onClick={() => setTimerMinutes(min)}
                      className={`text-xs px-2 py-1 rounded-lg transition-colors ${
                        timerMinutes === min ? 'bg-[#ff3621] text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700'
                      }`}>
                      {min}분
                    </button>
                  ))}
  ...
  return (
    <div className="min-h-full bg-gray-50 dark:bg-gray-950 pb-12">
      <div className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-4 py-3 sticky top-14 z-40">
        <div className="max-w-3xl mx-auto flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400">
            <span>답변: <span className="font-semibold text-gray-900 dark:text-white">{answeredCount}/{quizQuestions.length}</span></span>
          </div>
          <Timer totalSeconds={totalSeconds} remainingSeconds={remainingSeconds}
            onTick={handleTick} onExpire={handleExpire} running={timerRunning && !submitted} />
      ...
      <div className="max-w-3xl mx-auto px-4 pt-6">
        ...
        <QuestionCard question={q} questionIndex={currentIndex} totalQuestions={quizQuestions.length}
          selected={selected} onSelect={handleSelect} checked={false} onCheck={() => {}}
          showExplanation={false} onToggleExplanation={() => {}} mode="test" />
        ...
        <div className="mt-4 flex items-center justify-between">
          ...
          <span className="text-sm text-gray-500 dark:text-gray-400 font-medium">{currentIndex + 1} / {quizQuestions.length}</span>
        ...
        <div className="mt-6 flex flex-wrap gap-1.5">
          {quizQuestions.map((tq, i) => {
            ...
            return (
              <button key={tq.num} onClick={() => setCurrentIndex(i)} title={`Q${tq.num}`}
                className={`w-6 h-6 rounded text-xs font-medium transition-all ${
                  i === currentIndex ? 'bg-[#ff3621] text-white ring-2 ring-[#ff3621] ring-offset-1 dark:ring-offset-gray-950'
                  : ans ? 'bg-sky-500 text-white hover:bg-sky-600'
